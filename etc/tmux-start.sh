...
tmux -f "$TMUX_CONF" new-session -s $SESSION "bash --rcfile <(echo 'PROMPT_COMMAND=\"tmux send-keys -t $SESSION:0.2 \\\"$SCRIPT_TO_RUN\\\" C-m\"; \$PROMPT_COMMAND')" \; \
    ...
    resize-pane -U -t $SESSION:0.1 10 \; \
    select-pane -t 0 \; \
    ...
    attach
# Desactivar navegación de paneles
tmux unbind-key Up
tmux unbind-key Down
tmux unbind-key Left
tmux unbind-key Right
tmux select-pane -t 0  # aseguramos que el foco quede en panel izquierdo