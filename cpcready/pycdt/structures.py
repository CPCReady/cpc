...
"""
Estructuras de datos para el formato CDT (TZX) de Amstrad CPC
Implementación basada en el trabajo original de Javier Garcia
"""

import math
from .exceptions import CDTFormatError
...
class CDTHeader:
    ...
    def set(self, content):
        if len(content) < 10:
            raise CDTFormatError("header size is less than 10 bytes")
    ...
    def check(self):
        if "ZXTape!" not in self.title:
            raise CDTFormatError("CDT header title doesn't contain 'ZXTape!' text")
...
class BlockPause:
    ...
    def check(self):
        if self.pause < 0:
            raise CDTFormatError("negative pause time in Pause block")
